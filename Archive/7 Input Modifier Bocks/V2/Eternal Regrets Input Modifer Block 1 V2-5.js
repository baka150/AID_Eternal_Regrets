...
const modifier = (text) => {
  ...
  try {
    ...
    let sentiment = detectSentiment(text); // Added: Call sentiment here for conditional increment (coord with Block 3)
    if (text.trim() === '' || sentiment === 'neutral') {  // Added: Only increment on empty or neutral to prevent auto-ends on meaningful inputs
